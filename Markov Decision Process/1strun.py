...
for j in range(10):
    pos = start[i].copy()
    t.write("Start state: "+str(pos)+"\n")
    np.random.seed(j)
    swerve = np.random.random_sample(1000000)
    k=0
    reward=0.0
    while(not(pos[0]==goal[i][0] and pos[1]==goal[i][1])):
        t.write("Best move from: "+str(pos)+":  "+str(updatedPolicyMatrix[pos[0],pos[1]])+"\n")
        move = updatedPolicyMatrix[pos[0],pos[1]]
        t.write("Swerve: "+str(swerve[k])+"\n")
        if swerve[k] > 0.7:
            if swerve[k] > 0.8:
                if swerve[k] > 0.9:
        ...
        if(move==0):
            ...
            if(pos[1]!=s-1):
                pos[1]+=1    
        t.write("Next Position: "+str(pos)+"\n")
        reward+=rewardMatrix[pos[0],pos[1]]
        t.write("Current Reward: "+str(reward)+"\n")
        k=k+1
#    reward+=rewardMatrix[goal1.x,goal1.y]
    totalRewards+=reward    
    t.write("Total reward after one more iteration:************** "+str(totalRewards)+"\n")
print(np.floor(totalRewards/10))
